// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Persistence;

#nullable disable

namespace Persistence.Migrations
{
    [DbContext(typeof(HUB_Context))]
    [Migration("20231123094612_RelationM_850DocTypeSubLdgtype")]
    partial class RelationM850DocTypeSubLdgtype
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Domain.Common.GeneralSetup", b =>
                {
                    b.Property<int>("Code")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Code"));

                    b.Property<string>("AppPassword")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("MailApi")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("MailHost")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("MailPort")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Code");

                    b.ToTable("GeneralSetups");
                });

            modelBuilder.Entity("Domain.Entities.MainModule.Master.Branch", b =>
                {
                    b.Property<int>("BranchCode")
                        .HasColumnType("int");

                    b.Property<string>("Address")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BranchAraName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BranchEngName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BranchLogo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CompanyCode")
                        .HasColumnType("int");

                    b.Property<string>("Fax")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("LicenseNo")
                        .HasColumnType("int");

                    b.Property<int?>("Manager")
                        .HasColumnType("int");

                    b.Property<DateTime?>("OpenDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Telephone")
                        .HasColumnType("nvarchar(max)");

                    b.Property<TimeSpan?>("WorkTimeTo")
                        .HasColumnType("time");

                    b.Property<TimeSpan?>("workTimeFrom")
                        .HasColumnType("time");

                    b.HasKey("BranchCode");

                    b.HasIndex("CompanyCode");

                    b.ToTable("Branch");
                });

            modelBuilder.Entity("Domain.Entities.MainModule.Master.Company", b =>
                {
                    b.Property<int>("CompanyCode")
                        .HasColumnType("int");

                    b.Property<string>("CompanyArName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CompanyEnName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("CompanyCode");

                    b.ToTable("Company");
                });

            modelBuilder.Entity("Domain.Entities.MainModule.Master.Customer", b =>
                {
                    b.Property<int>("CustCode")
                        .HasColumnType("int");

                    b.Property<int?>("BasicCurrencyCode")
                        .HasColumnType("int");

                    b.Property<string>("ContactPerson")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ContactPersonEmail")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CountryCode")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CountryISO")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("CreateDateAndTime")
                        .HasColumnType("datetime");

                    b.Property<int?>("CreateUserId")
                        .HasColumnType("int");

                    b.Property<string>("CustAddress")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CustLatAddress")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CustLatName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CustName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("DateAndTime")
                        .HasColumnType("datetime");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Fax")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Logo")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Mobile")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool?>("NotActive")
                        .HasColumnType("bit");

                    b.Property<string>("TaxId")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TaxType")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Telephone")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("UserId")
                        .HasColumnType("int");

                    b.HasKey("CustCode");

                    b.ToTable("Customer");
                });

            modelBuilder.Entity("Domain.Entities.MainModule.Master.M_850DocType", b =>
                {
                    b.Property<int>("Kind")
                        .HasColumnType("int");

                    b.Property<int>("DocTypeCode")
                        .HasColumnType("int");

                    b.Property<DateTime?>("CreateDateAndTime")
                        .HasColumnType("datetime");

                    b.Property<int?>("CreateUserId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("DateAndTime")
                        .HasColumnType("datetime");

                    b.Property<string>("DefaultAdmin")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DefaultRead")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DefaultWrite")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DocTypeAraName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DocTypeLatName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool?>("NonActive")
                        .HasColumnType("bit");

                    b.Property<int>("Serial")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Serial"));

                    b.Property<int>("SubLdgTypeCode")
                        .HasColumnType("int");

                    b.Property<string>("Symbol")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("UserId")
                        .HasColumnType("int");

                    b.HasKey("Kind", "DocTypeCode");

                    b.ToTable("M_850DocType");
                });

            modelBuilder.Entity("Domain.Entities.MainModule.Master.M_850DocTypeSubLdgType", b =>
                {
                    b.Property<int>("Serial")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Serial"));

                    b.Property<DateTime?>("CreateDateAndTime")
                        .HasColumnType("datetime");

                    b.Property<int?>("CreateUserId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("DateAndTime")
                        .HasColumnType("datetime");

                    b.Property<int>("DocTypeCode")
                        .HasColumnType("int");

                    b.Property<int>("Kind")
                        .HasColumnType("int");

                    b.Property<int>("SubLdgTypeCode")
                        .HasColumnType("int");

                    b.Property<int?>("UserId")
                        .HasColumnType("int");

                    b.HasKey("Serial");

                    b.HasIndex("SubLdgTypeCode");

                    b.HasIndex("Kind", "DocTypeCode")
                        .IsUnique();

                    b.ToTable("M_850DocTypeSubLdgTypes");
                });

            modelBuilder.Entity("Domain.Entities.MainModule.Master.SubLdgType", b =>
                {
                    b.Property<int>("SubLdgTypeCode")
                        .HasColumnType("int");

                    b.Property<DateTime?>("CreateDateAndTime")
                        .HasColumnType("datetime");

                    b.Property<int?>("CreateUserId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("DateAndTime")
                        .HasColumnType("datetime");

                    b.Property<bool?>("NotActive")
                        .HasColumnType("bit");

                    b.Property<string>("SubLdgTypeAraName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("SubLdgTypeLatName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("UserId")
                        .HasColumnType("int");

                    b.HasKey("SubLdgTypeCode");

                    b.ToTable("SubLdgType");
                });

            modelBuilder.Entity("Domain.Entities.ReservationModule.WeekDay", b =>
                {
                    b.Property<int>("DayCode")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("DayCode"));

                    b.Property<string>("DayLatName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DayName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("DayCode");

                    b.ToTable("WeekDays");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.ActionTypeUser", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ActionTypeName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.HasKey("Id");

                    b.ToTable("ActionTypeUser");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.City", b =>
                {
                    b.Property<int>("CountryCode")
                        .HasColumnType("int");

                    b.Property<int>("CityCode")
                        .HasColumnType("int");

                    b.Property<string>("CityName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.HasKey("CountryCode", "CityCode");

                    b.ToTable("Citys");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.Country", b =>
                {
                    b.Property<int>("CountryCode")
                        .HasColumnType("int");

                    b.Property<string>("CountryName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("CountryphoneCode")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("CreateDateAndTime")
                        .HasColumnType("datetime");

                    b.Property<int?>("CreateUserId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("DateAndTime")
                        .HasColumnType("datetime");

                    b.Property<string>("PhoneCode")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("UserId")
                        .HasColumnType("int");

                    b.HasKey("CountryCode");

                    b.ToTable("Countrys");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.Group", b =>
                {
                    b.Property<int>("GroupId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("GroupId"));

                    b.Property<DateTime?>("CreateDateAndTime")
                        .HasColumnType("datetime");

                    b.Property<int?>("CreateUserId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("DateAndTime")
                        .HasColumnType("datetime");

                    b.Property<string>("GroupLatName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("GroupName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<int?>("UserId")
                        .HasColumnType("int");

                    b.HasKey("GroupId");

                    b.ToTable("Groups");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.GroupPermission", b =>
                {
                    b.Property<int>("GroupCode")
                        .HasColumnType("int");

                    b.Property<decimal>("ProgId")
                        .HasColumnType("decimal(18, 0)");

                    b.Property<bool?>("Delete")
                        .HasColumnType("bit");

                    b.Property<bool?>("Edit")
                        .HasColumnType("bit");

                    b.Property<bool?>("Insert")
                        .HasColumnType("bit");

                    b.Property<bool?>("Print")
                        .HasColumnType("bit");

                    b.Property<bool?>("Read")
                        .HasColumnType("bit");

                    b.HasKey("GroupCode", "ProgId");

                    b.ToTable("GroupPermission");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.PrgPer", b =>
                {
                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<decimal>("ProgId")
                        .HasColumnType("decimal(18, 0)")
                        .HasComment("كود الشاشة");

                    b.Property<bool?>("Delete")
                        .HasColumnType("bit")
                        .HasComment("صلاحية حذف بيانات");

                    b.Property<bool?>("Edit")
                        .HasColumnType("bit")
                        .HasComment("صلاحية تعديل بيانات");

                    b.Property<bool?>("Insert")
                        .HasColumnType("bit")
                        .HasComment("صلاحية إدخال بيانات");

                    b.Property<bool?>("Print")
                        .HasColumnType("bit")
                        .HasComment("صلاحية الطباعة");

                    b.Property<bool?>("Read")
                        .HasColumnType("bit")
                        .HasComment("صلاحية قراءة بيانات");

                    b.HasKey("UserId", "ProgId");

                    b.ToTable("PrgPer");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.Program", b =>
                {
                    b.Property<decimal>("ProgId")
                        .HasColumnType("decimal(18, 0)");

                    b.Property<string>("ArabicName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("FormName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<bool?>("HaveDelete")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValueSql("((1))");

                    b.Property<bool?>("HaveEdit")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValueSql("((1))");

                    b.Property<bool?>("HaveInsert")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValueSql("((1))");

                    b.Property<bool?>("HavePrint")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValueSql("((1))");

                    b.Property<bool?>("HaveRead")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValueSql("((1))");

                    b.Property<string>("LatinName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Parameters")
                        .IsRequired()
                        .HasMaxLength(250)
                        .IsUnicode(false)
                        .HasColumnType("varchar(250)");

                    b.Property<decimal?>("ParentId")
                        .HasColumnType("decimal(18, 0)")
                        .HasColumnName("ParentID");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)")
                        .HasColumnName("URL");

                    b.HasKey("ProgId");

                    b.ToTable("Programs");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<bool?>("Approval")
                        .HasColumnType("bit");

                    b.Property<DateTime?>("BirthDay")
                        .HasColumnType("Date");

                    b.Property<int?>("CountryCode")
                        .HasColumnType("int");

                    b.Property<DateTime?>("CreateDateAndTime")
                        .HasColumnType("datetime");

                    b.Property<int?>("CreateUserId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("DateAndTime")
                        .HasColumnType("datetime");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int?>("Gender")
                        .HasColumnType("int");

                    b.Property<int?>("GroupId")
                        .IsRequired()
                        .HasColumnType("int");

                    b.Property<string>("Mobile")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("Mobile1")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool?>("NotActive")
                        .HasColumnType("bit");

                    b.Property<byte[]>("PasswordHash")
                        .HasColumnType("varbinary(max)");

                    b.Property<byte[]>("PasswordSolt")
                        .HasColumnType("varbinary(max)");

                    b.Property<string>("PhotoUserPath")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("UserId")
                        .HasColumnType("int");

                    b.Property<string>("UserName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<int?>("UserType")
                        .HasColumnType("int");

                    b.Property<string>("address")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .IsUnique()
                        .HasFilter("[Email] IS NOT NULL");

                    b.HasIndex("GroupId");

                    b.HasIndex("UserName")
                        .IsUnique();

                    b.ToTable("Users");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.UserLog", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("ActionType")
                        .HasColumnType("int");

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime");

                    b.Property<string>("IPaddress")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("UsersLog");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.VerificationCode", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int>("Code")
                        .HasColumnType("int");

                    b.Property<bool>("IsUsed")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("UserId");

                    b.ToTable("VerificationCodes");
                });

            modelBuilder.Entity("Domain.Entities.MainModule.Master.Branch", b =>
                {
                    b.HasOne("Domain.Entities.MainModule.Master.Company", "Company")
                        .WithMany("branches")
                        .HasForeignKey("CompanyCode")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Company");
                });

            modelBuilder.Entity("Domain.Entities.MainModule.Master.M_850DocTypeSubLdgType", b =>
                {
                    b.HasOne("Domain.Entities.MainModule.Master.SubLdgType", "subLdgType")
                        .WithMany("M_850DocTypeSubLdgTypes")
                        .HasForeignKey("SubLdgTypeCode")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Domain.Entities.MainModule.Master.M_850DocType", "m_850DocType")
                        .WithMany("M_850DocTypeSubLdgTypes")
                        .HasForeignKey("Kind", "DocTypeCode")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("m_850DocType");

                    b.Navigation("subLdgType");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.City", b =>
                {
                    b.HasOne("Domain.Entities.SecurityModule.Master.Country", "country")
                        .WithMany("Citys")
                        .HasForeignKey("CountryCode");

                    b.Navigation("country");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.User", b =>
                {
                    b.HasOne("Domain.Entities.SecurityModule.Master.Group", "Groups")
                        .WithMany("Users")
                        .HasForeignKey("GroupId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Groups");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.VerificationCode", b =>
                {
                    b.HasOne("Domain.Entities.SecurityModule.Master.User", "User")
                        .WithMany("VerificationCodes")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Domain.Entities.MainModule.Master.Company", b =>
                {
                    b.Navigation("branches");
                });

            modelBuilder.Entity("Domain.Entities.MainModule.Master.M_850DocType", b =>
                {
                    b.Navigation("M_850DocTypeSubLdgTypes");
                });

            modelBuilder.Entity("Domain.Entities.MainModule.Master.SubLdgType", b =>
                {
                    b.Navigation("M_850DocTypeSubLdgTypes");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.Country", b =>
                {
                    b.Navigation("Citys");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.Group", b =>
                {
                    b.Navigation("Users");
                });

            modelBuilder.Entity("Domain.Entities.SecurityModule.Master.User", b =>
                {
                    b.Navigation("VerificationCodes");
                });
#pragma warning restore 612, 618
        }
    }
}
